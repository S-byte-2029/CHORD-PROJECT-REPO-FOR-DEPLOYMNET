...
MASTER_SCHEMES_DATASET = [
    ...
    },
    {
        ...
        "estimated_benefit": "₹5,000 / month lifetime",
    ...
    {
        "name": "Uttarakhand Mukhyamantri Swarojgar Yojana (MSY)",
        "slug": "uttarakhand-mukhyamantri-swarojgar-yojana",
        "category": "State Welfare",
        "ministry": "Department of Industries, Uttarakhand",
        "gov_level": "State Government",
        "state_coverage": "Uttarakhand",
        "objective": "Encourage self-employment and reverse migration by providing credit-linked subsidies to youth in hilly and plain districts.",
        "description": "Subsidy of 15% to 25% on project costs up to ₹25 lakh for manufacturing and ₹10 lakh for service sector enterprises.",
        "benefits_summary": "15% to 25% capital subsidy up to ₹6.25 lakh with 0% margin money requirement for SC/ST/women.",
        "benefits": [
            {"title": "Capital Subsidy", "desc": "Up to 25% (max ₹6.25 Lakh) government subsidy.", "icon": "dollar-sign"},
            {"title": "Bank Financing", "desc": "Collateral-free loans under CGTMSE guarantee.", "icon": "shield"}
        ],
        "eligibility": [
            "Permanent resident / domicile holder of Uttarakhand.",
            "Minimum age 18 years with no formal minimum education bar.",
            "Preference for returned migrant workers and local youth."
        ],
        "documents": ["Domicile Certificate of Uttarakhand", "Aadhaar Card", "Detailed Project Report (DPR)", "Bank Account Details"],
        "deadline": "31 Mar 2027",
        "official_link": "https://msy.uk.gov.in/",
        "estimated_benefit": "₹6,25,000 capital subsidy",
        "target_occupations": ["Business Owner", "Self Employed", "Unemployed"],
        "max_income": 1500000
    }
]
...
def ingest_batch(batch_records):
    """
    Ingests a list of scheme dictionaries in a single database transaction.
    Batch size chunking: 100 rows per batch.
    """
    created_count = 0
    updated_count = 0
        ...
        for record in batch_records:
            ...
            if not scheme:
                ...
                scheme = Scheme.objects.create(
                    ...
                    name=name,
                    category=category,
                    ministry=record.get('ministry', 'Government of India'),
                ...
                scheme.ministry = record.get('ministry', scheme.ministry)
        ...
        if file_path.endswith('.json'):
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    for key in ['benefits', 'eligibility', 'documents', 'target_occupations']:
